...
gnome-terminal \
  --tab-with-profile=halt -e '/bin/bash -c "\
    cd ~/catkin_ws/src/mybot/mybot_gazebo/launch; \
    roslaunch mybot_and_op3.launch;\
  " ' \
  --tab-with-profile=halt -e '/bin/bash -c "\
    sleep 5;\
  ...
  --tab-with-profile=halt -e '/bin/bash -c "\
    sleep 17;\
    echo Starting to relay topic /mybot/camera/image_raw to /archie/camera/image_raw;\
